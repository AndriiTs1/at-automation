...
export default function MobileNeedsAttention() {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden rounded-xl border border-border bg-surface p-3 shadow-sm shadow-black/5">
        ...
        {NEEDS_ATTENTION.map((item) => {
          ...
          return (
            <button
              key={item.key}
              type="button"
              className="flex items-center gap-2.5 py-2.5 text-left transition-colors first:pt-1 last:pb-1 hover:bg-black/[0.03]"
            >
