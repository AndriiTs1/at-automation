  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden rounded-2xl border border-slate-200/70 bg-white p-2.5 shadow-demo-card">
      <div className="mb-1 flex shrink-0 items-center justify-between">
        <h4 className="truncate text-xs font-semibold tracking-tight text-slate-900">{t("title")}</h4>
      ...
      <div className="flex flex-1 flex-col divide-y divide-slate-100 overflow-y-auto">
        {NEEDS_ATTENTION.map((item) => {
          ...
          return (
            <button
              key={item.key}
              type="button"
              className="flex items-center gap-2 py-2 text-left transition-colors first:pt-0.5 last:pb-0.5 hover:bg-slate-50"
            >
              <span className={`h-2 w-2 shrink-0 self-start rounded-full ${SEVERITY_DOT[item.severity]}`} />
              <span className="min-w-0 flex-1">
                <span className="block text-[12px] leading-snug font-semibold text-slate-900">{title}</span>
                <span className="mt-0.5 block text-[11px] leading-snug text-slate-500">{meta}</span>
                <span className="mt-0.5 block text-[10px] leading-snug text-slate-400">
                  {t("ownerLabel", { owner })} · {note}
                </span>
              </span>
              <ChevronDownIcon className="h-3.5 w-3.5 shrink-0 -rotate-90 self-center text-slate-300" />
            </button>
          );
        })}
      </div>
