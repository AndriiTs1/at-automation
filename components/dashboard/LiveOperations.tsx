      ...
      <div className="flex flex-1 flex-col gap-2 overflow-y-auto @3xl:gap-2.5">
        {LIVE_OPERATIONS.map((event) => (
          <div key={event.time + event.titleKey} className="flex gap-2 text-[11px] @3xl:text-xs">
            <span className="shrink-0 pt-0.5 font-mono text-[10px] text-slate-400 @3xl:text-[11px]">{event.time}</span>
            <div className="min-w-0 flex-1">
              <p className="font-medium text-slate-900 @lg:truncate">{t(`events.${event.titleKey}`, event.titleParams)}</p>
              <div className="mt-0.5 flex flex-col gap-0.5">
                {event.steps.map((step) => (
                  <p key={step.key} className="truncate text-[10px] text-slate-500 @3xl:text-[11px]">
                    <span className="text-slate-300">→</span> {t(`steps.${step.key}`, "params" in step ? step.params : undefined)}
                  </p>
                ))}
              </div>
            </div>
